...
app.get('/api/seats', async (req, res) => {
    try {
        ...
        // Group seats back into rows for the frontend
        const rows = {};
        event.seats.forEach(seat => {
        ...
        const layout = Object.keys(rows).sort().map(rowKey => ({
            row: rowKey,
...
app.post('/api/book', async (req, res) => {
    ...
    try {
        ...
        // We are looking for seats in the subdocument array that match the IDs
        ...
        // We update the specific subdocuments.
        // Mongoose subdoc updates:
        seatIds.forEach(id => {
        ...
        // In a standalone mongo, this save is atomic at the document level.
        // Since all seats are inside one Event document, this effectively locks the event during write.
        await event.save();
